...
class HubertFeatureReader(object):
    ...
    def load_feature(self, mix_name, ref_len=None):
        ...
        print("MIX _ NAME = ", mix_name)
        video_feats = self.load_image(video_fn)

        audio_fn = audio_fn.split(':')[0]

        # Load audio using torchaudio to handle resampling and channel conversion
        wav_data, sample_rate = torchaudio.load(audio_fn)
        ...
        wav_data = wav_data.squeeze(0).numpy()
        
        assert len(wav_data.shape) == 1, "Audio must be mono!"
        
        audio_feats = logfbank(wav_data, samplerate=16000).astype(np.float32)
